...
Just for a sanity check, request an info for a first staker
```
sfcc.stakersNum() // if everything is allright, will return a non-zero value
```
...
tx = sfcc.createStake("0x", {from:YOUR_ADDRESS, value: "3175000000000000000000000"}) // 3175000.0 FTM
```
...
```
#!/bin/sh
$HOME/go/src/github.com/Fantom-foundation/go-lachesis/build/lachesis --nousb --validator 0xAddress --unlock 0xAddress --password /path/to/password
```
